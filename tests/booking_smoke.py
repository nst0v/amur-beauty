...
try:
 with sync_playwright() as p:
  ...
  for width,height in [(320,740),(360,800),(390,844),(430,932),(768,1024),(1024,900),(1440,1000)]:
   ...
   # This fixture isolates layout and URL tests. A real network calendar is checked below.
   ...
   assert not page.evaluate('document.documentElement.scrollWidth > innerWidth + 1'),f'Page overflow at {width}'
   ...
   src=page.locator('#booking-calendar').get_attribute('src');params=parse_qs(urlparse(src).query)
   ...
   page.locator('#catalog-search').fill('неттакойуслуги')
  ...
  # Read-only network test: the real provider calendar, not the fixture above.
  ...
  def guard(route):
   ...
   if 'dikidi.net' in u:
    ...
    if request.method not in ('GET','HEAD','OPTIONS') or 'reservation' in u or '/newrecord/final' in u:
     route.abort();return
   route.continue_()
  page.route('**/*',guard)
  page.goto('http://127.0.0.1:8767/',wait_until='domcontentloaded')
  ...
  results['liveCalendar']={'status':'passed','parentUrl':page.url,'frameUrl':page.locator('#booking-calendar').get_attribute('src'),'text':text,'appointmentActionsPerformed':False,'networkRequests':requests}
  print('PASS live embedded calendar: actual service and master, no appointment submitted',flush=True)
  browser.close()
except Exception:
